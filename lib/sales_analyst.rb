...
class SalesAnalyst
  ...
  def get_failed_result
   failures = @engine.selected_transaction_by_result(:failed)
   collector = []
   failures.each do |key,value|
      collector << value.invoice_id
    end
    collector
  end

  def failed_merchants
    get_failed_result.flat_map do |invoice_id|
      @engine.failed_merchants_by_ids(invoice_id)[0][1].merchant_id
    end
  end

  def extract_pending
    extract_merchant_ids.map do |merchant_id|
      @engine.merchants.find_by_id(merchant_id)
    end
  end

  def extract_failed
    failed_merchants.map do |merchant_id|
      @engine.merchants.find_by_id(merchant_id)
    end
  end

  def merchants_with_pending_invoices
    (extract_pending + extract_failed).uniq
  end
